function []=plot_evaluation(evaluation_data, evaluation_data_bf)
...
t_e=zeros(num_pairs, 1);

r_e_bf=zeros(num_pairs, 1);
t_e_bf=zeros(num_pairs, 1);
...
for i=1: num_pairs;
    ...
    t_mag(i)=evaluation_data(i).trans_mag;
    
    if ~isempty(evaluation_data(i).rot_error)
        r_e(i)=norm(evaluation_data(i).rot_error);
        t_e(i)=norm(evaluation_data(i).trans_error);
    ...
    if ~isempty(evaluation_data_bf(i).rot_error)
      
        r_e_bf(i)=norm(evaluation_data_bf(i).rot_error);
        t_e_bf(i)=norm(evaluation_data_bf(i).trans_error);
...
figure
ours_rotation=stability_plot(r_e, r_mag, '-bd');
hold on
ours_translation=stability_plot(t_e, r_mag, '-b*');

CODE_rotation=stability_plot(r_e_bf, r_mag, '--rd');
CODE_translation=stability_plot(t_e_bf, r_mag, '--r*');
