...
int  SiftGPU::RunSIFT_CU(int width, int height, const void * data, unsigned int gl_format, unsigned int gl_type)
{
	...
	if (width > 0 && height >0 && data != NULL)
	{
		...
		//try downsample the image on CPU
		GlobalUtil::StartTimer("Upload Image data");
		if (_texImage->SetImageData_CU(width, height, data, gl_format, gl_type))
		{
			_image_loaded = 2; //gldata;
			GlobalUtil::StopTimer();
			_timing[0] = GlobalUtil::GetElapsedTime();
...
void SiftGPU::GetFeatureVector(float * keys, float * descriptors)
{
	//	keys.resize(_pyramid->GetFeatureNum());
	if (GlobalUtil::_DescriptorPPT)
	{
		//	descriptors.resize(128*_pyramid->GetFeatureNum());
		_pyramid->CopyFeatureVector(&keys[0], &descriptors[0]);
	}
	else
	{
		//descriptors.resize(0);
		_pyramid->CopyFeatureVector(&keys[0], NULL);
	}
}
...
int SiftGPU::firstOctave() {
	return GlobalUtil::_octave_min_default;
}
