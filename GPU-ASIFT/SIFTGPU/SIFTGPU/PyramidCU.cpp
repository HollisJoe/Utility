...
void PyramidCU::ConvertInputToCU_CU(GLTexInput* input)
{
	int ws = input->GetImgWidth(), hs = input->GetImgHeight();
	TruncateWidth(ws);
	//copy the input image to pixel buffer object
	...
	{
		_inputTex->InitTexture(ws, hs, 1);
		_inputTex->CopyFromHost_CU(input->_pixel_data);
	}
	else
	{
		if (_bufferPBO == 0) glGenBuffers(1, &_bufferPBO);
...
void PyramidCU::BuildPyramid_CU(GLTexInput * input)
{
