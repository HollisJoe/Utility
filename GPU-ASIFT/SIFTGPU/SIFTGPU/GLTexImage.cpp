...
int GLTexInput::SetImageData_CU(int width, int height, const void * data,
	unsigned int gl_format, unsigned int gl_type)
{
	int simple_format = IsSimpleGlFormat(gl_format, gl_type);//no cpu code to handle other formats
	int ws, hs, done = 1;
	...
	if (_down_sampled > 0) {
		cout << "SiftGPU automatic downsampling engaged, cannot use DMA mode ..." << endl;
		return 0;
	}
...
int GLTexInput::LoadImageFile(char *imagepath, int &w, int &h )
{
	...
	if(ilLoadImage((const wchar_t*)imagepath))
	{
