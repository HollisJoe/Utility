...
public:
	...
	int					_computeCap_Major;
	int					_computeCap_Minor;
	int					_globalMem_MB;
	float			    _timing[10];
...
public:
	...
	SIFTGPU_EXPORT virtual void GetFeatureVector(float* keys, float* descriptors);
	...
	//Using image data already inside the device -- WANG FAN
	SIFTGPU_EXPORT virtual int  RunSIFT_CU(int width, int height, const void * data,
										unsigned int gl_format, unsigned int gl_type);
	//run SIFT on current image (specified by arguments), or processing the current image again
	SIFTGPU_EXPORT virtual int  RunSIFT();
	SIFTGPU_EXPORT virtual int  RunSIFT_CU();
...
public:
	...
	SIFTGPU_EXPORT int firstOctave();
};
